...
import { ArrowLeft, MapPin, Users, Clock, Gauge, ImageOff } from 'lucide-react';
    ...
    <div className="max-w-lg md:max-w-3xl mx-auto p-4">
      ...
      {baseGame && (
        <Link
          ...
          className="block text-sm text-gray-500 dark:text-gray-400 mb-3 hover:text-gray-800 dark:hover:text-gray-200"
        >
          본판: {baseGame.title} →
        </Link>
      )}

      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden md:flex">
        <div className="h-52 md:h-auto md:w-72 md:shrink-0 bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
          {game.imageUrl ? (
        ...
        <div className="p-5 flex-1">
          ...
          <div className="flex gap-2 mt-4">
            <div className="flex-1 flex flex-col items-center gap-1 bg-gray-50 dark:bg-gray-800 rounded-lg py-2.5 text-xs text-gray-600 dark:text-gray-300">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed mt-4">{game.description}</p>
        </div>
      </div>

      {game.expansionIds.length > 0 && (
        <div className="mt-6">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-3">확장판 목록</p>
                ...
                <Link key={expId} href={`/games/${expansion.id}`} className="flex flex-col gap-1.5">
                  <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                    {expansion.imageUrl ? (
                      ...
                      <ImageOff size={20} className="text-gray-300 dark:text-gray-600" />
                    )}
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-300 text-center">{expansion.title}</span>
                </Link>
              );
            })}
          </div>
